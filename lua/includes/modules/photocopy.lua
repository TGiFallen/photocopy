...
function svGhoster:SendInitializeInfo()
	//GetTime()
	MsgN("SENDING INFO")
	net.Start("photocopy_ghost_init" )
		net.WriteEntity( self:GetGhostController() )
		net.WriteFloat( self.offset.x )
		net.WriteFloat( self.offset.y )
		net.WriteFloat( self.offset.z )
	net.Broadcast()
	//net.Send( self.ply )
	self:SetNext(0 , self.SendGhostInfo)
	//GetTime()
end
util.AddNetworkString("photocopy_ghost_init")
--ghost info
function svGhoster:SendGhostInfo()
	...
	for i = 1 , self.SendRate do        
		...
		net.Start("photocopy_ghost_info" , self.ply )
			net.WriteString(entData.Model)
			-- postion
...
util.AddNetworkString("photocopy_ghost_info")
